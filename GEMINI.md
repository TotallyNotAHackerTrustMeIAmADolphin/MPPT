...
- **USB DFU Auto-Reset**: Detects 1200 bps baud rate requests to automatically jump to the system memory bootloader.
...
- **Memory Preservation**: Use the `.noinit` section for variables that must survive a software reset (e.g., `dfu_magic`).
...
- `Core/Src/main.c`: Core application logic and jump sequences.
- `setup_cubemx_env_auto.py`: Automated build environment bridge.
- `STM32F072RBTX_FLASH.ld`: Linker script containing the `.noinit` section.
